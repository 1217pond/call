document.getElementById("login").addEventListener("click",() => {
  ...
  apikey = decrypt(encrypted_apikey,license.replace(/-/g,""));
  console.log(apikey);
  if(apikey){
    pop_up("ログインしました。");
...
function prep(){
  ...
  const peer = new Peer({//peerサーバーに接続
    key: apikey,
    debug: 1
  });

  peer.on("open",(id) => {//peer idを取得
    peer_id = id;
    console.log(id);
  });
  ...
  navigator.mediaDevices.getUserMedia({video: true, audio: true}).then( stream => {//mediastreamを取得
    ...
    // 失敗時にはエラーログを出力
    pop_up("マイク等の許可をしてください。何度も表示される場合はサイトを再読み込みしてください。",true);
    console.log(error);
    return;
  });
}
...
let main_pass;
let peer_id;
let encrypted_apikey = "U2FsdGVkX19syH8tqsmMTHceDBu7lqZVZF/Br+TwN23aKXB6edJuitxR0BHBKmVzN1nT6Puz4LH95F8A7aJm0w=="; 
let apikey;
